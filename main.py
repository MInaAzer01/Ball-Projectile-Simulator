...
class Ball:
    ...
    def draw_ball(self):
        pg.draw.circle(surface=self.win, color=RED, center=current_mouse_pos, radius=BALL_RADIUS)

    def draw_line(self, mouse_pos):
        pg.draw.circle(surface=self.win, color=BLACK, center=self.pos, radius=2)
    ...
    def draw_ball_path(self, ball_loc_list):
        if ball_loc_list:
            for pos in ball_loc_list:
                pg.draw.circle(surface=self, color=RED, center=pos, radius=2)
...
win = pg.display.set_mode((WIDTH, HIGHT))
clock = pg.time.clock(FPS)
current_ball_pos = INITIAL_BLL_POS
    ...
    def exit():
        ...
        while 1:
            ...
            if launch:
                ...
                if current_ball_pos[1] <= INITIAL_BLL_POS[1]:
                    x, y = start_ball_pos
                    ...
                    launch = False
                    current_ball_pos = (current_ball_pos[0], INITIAL_BLL_POS[1])
        ...
        for event in pg.event.get():
            ...
            elif event.type == pg.MOUSEBUTTONDOWM:
